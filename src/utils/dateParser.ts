...
export function parseDateHeader(header: string): Date {
  try {
    ...
    const dateStr = `${dayMonth} ${currentYear}`;
    ...
    // Handle year rollover: MarketWatch only shows future events, so if we're in Nov/Dec
    // and see a month before the current one, it must be next year (e.g., "FEB" in November = Feb next year)
    if (parsedDate.getMonth() < currentMonth && currentMonth >= 10) {
      const nextYearStr = `${dayMonth} ${currentYear + 1}`;
      const nextYearDate = parse(nextYearStr, 'MMMM d yyyy', new Date());
      if (isValid(nextYearDate)) {
        return nextYearDate;
      }
    }
